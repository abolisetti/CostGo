...
def moveX(mapX, mapY, badGuys, badGuyRects, shelfRects, rects, charRect, shelfItemRects):
    ...
    if keys[K_RIGHT] or keys[K_d]:
        newMove = RIGHT
        if moveCheck(12, 0, rects, charRect):#A checking function to see if there are objects infront of character
            mapX -= dist
            totalX -= dist
        ...
        newMove = LEFT
        if moveCheck(-12, 0, rects, charRect):
            mapX += dist
            totalX += dist
        ...
        #start = False
        newMove = UP
        if moveCheck(0, -12, rects, charRect):
            mapY += dist
    ...
    rects = [charRect] + badGuyRects + shelfRects
    return mapX, mapY, totalX, totalY, badGuys, badGuyRects, shelfRects, rects, charRect, shelfItemRects
...
def moveCheck(x, y, rects, charRect):
    """Checks to see if player has collided with an item"""
    #move checking for main guy
    #creates new rect, runs it by all rects, and if there will be collision ins a certain direction, its not allowed to go that way
...
def moveY(guy, guyRect, rects):
    ...
    if randint(1,200)==1:#So they turn sometimes but tehy shoudlnt turn all the time
        guy[DIR]=randint(0,3)
    if guy[DIR] == RIGHT:
        if badGuyCollide(guyRect, checkDist, 0, rects):#Function for checking if the guy will collide with an object, if so hell turn
            guy[DIR] = randint(0,3)
        else:
            guy[X]+=5
            
    if guy[DIR] == LEFT:
        if badGuyCollide(guyRect, -checkDist, 0, rects):
            guy[DIR] = randint(0,3)
        else:
            guy[X]-=5
            
    if guy[DIR] == UP:
        if badGuyCollide(guyRect, 0, -checkDist, rects):
            guy[DIR] = randint(0,3)
        else:
            guy[Y]-=5
...
def moveBadGuys(badGuys, badGuyRects, rects):#moves them all through a loop and runs through a parallel list
    """Goes through every bad guy and moves them around map"""
    
    
    for i in range(len(badGuys)):
        moveY(badGuys[i], badGuyRects[i], rects)#using the move y function on each badguy
...
def badGuyCollide(guy, x, y, rects):
...
def generateShoppingList(shelfItems): #this creates the first instance of the shopping list and only runs once
    ...
    indices=sample(range(0,150),20) #Creates a list of 20 indices, then 20 items
        ...
        for i in profileRects:
            if i.collidepoint(mx,my):
                ...
                if mb[0] == 1:
                    ...
                    return profileRects.index(i)+1, username, "Instructions"
...
def resetVals():
    ...
    #All the Rects
    #rects = [charRect]+shelfRects#keeps track of the rects i have now
    
    
    display.flip()

    badGuys, badGuyRects, rects = badGuyCreate(400, rects, pics)
...
def badGuyCreate(num, rects, pics):
    ...
    for i in range(num):
        ...
        if collide == False:
            ...
            rects.append(guyRect)

    return badGuys, badGuyRects, rects
...
def rectDraw(lst, color = 0):#draws a list of rects so its easier
    for i in lst:
...
def runGame():#the fucntion that actually runs the game
    global badGuys, badGuyRects, rects, mapX, mapY, shelfRects, shelfItems, shelfItemRects, charRect, frame2, shoppingList, elapsed, page,timeWithoutPause, userNum, username
...
##            resetVals()
        ...
        moveBadGuys(badGuys, badGuyRects, rects)
...
##                usernamesFile.close()
        ...
        mapX, mapY, totalX, totalY, badGuys, badGuyRects, shelfRects, rects, charRect, shelfItemRects = moveX(mapX, mapY, badGuys, badGuyRects, shelfRects, rects, charRect, shelfItemRects)
